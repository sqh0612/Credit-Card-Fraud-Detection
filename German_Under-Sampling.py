...
# 数据准备
print()
print("German-Credit-Data View")
print("--------------------------------------------------------------")
df_german = pd.read_csv("Data/german-numeric.csv", header=None)
df1 = df_german.iloc[:, 0:]
print("Original German-Credit-Data : No. of positive samples = {}, No. of negative samples = {}".format(
    df1.loc[df1.iloc[:, -1] == 2, :].shape[0], df1.loc[df1.iloc[:, -1] == 1, :].shape[0]))
legit = df_german[df1.iloc[:, -1] == 1]   # 1 = Good__阴性
fraud = df_german[df1.iloc[:, -1] == 2]   # 2 = Bad__阳性
legit_sample = legit.sample(n=300)
new_dataset = pd.concat([legit_sample, fraud], axis=0)
print("Under-Sampling German-Credit-Data : No. of positive samples = {}, No. of negative samples = {}".format(
    new_dataset.loc[new_dataset.iloc[:, -1] == 2, :].shape[0], new_dataset.loc[new_dataset.iloc[:, -1] == 1, :].shape[0]))
print("--------------------------------------------------------------")
print()

X_train_german, X_test_german, y_train_german, y_test_german = train_test_split(new_dataset.iloc[:, 0:-1], new_dataset.iloc[:, -1],
                                                                                test_size=0.30, random_state=0)
y_train_german[y_train_german == 1] = 0    # 1 = Good__阴性
y_train_german[y_train_german == 2] = 1    # 2 = Bad__阳性
...
def KNN(X_train, y_train, X_test, y_test, metric, n_splits, random_state, num_features, print_info, dataset):
    neighbours = [i for i in range(2, 10)]  # 从3-10中找出最佳的n，要保证在这区间有出现峰值，否则扩大区间
...
for algorithm in algorithms:
    ...
    time_taken_temp = []
    # 最后一个参数为1则输出F1图像，最后一个参数为0则输出AUC值
    if algorithm == 'bay':
        for training_size in training_sizes:
            ...
            if training_size == 1.0:
                ...
                test_error, temp_time2 = bay(X_train_temp, y_train_temp, X_test_german, y_test_german, recall_score, 5,
                                             0, 15, 1, 0)
        ...
        time_taken['Naive Bayes'] = time_taken_temp
        ...
        plt.savefig("German-Credit-Data-NB.png")
        plt.close()
        ...
        for training_size in training_sizes:
            X_train_temp, y_train_temp = split_data(X_train_german, y_train_german, training_size, 6)
            if training_size == 1.0:
                train_error, temp_time1 = KNN(X_train_temp, y_train_temp, X_train_temp, y_train_temp, recall_score, 5,
                                              1, 15, 0, 0)
                error_rate_training_data.append(train_error)
                test_error, temp_time2 = KNN(X_train_temp, y_train_temp, X_test_german, y_test_german, recall_score, 5,
                                             1, 15, 1, 0)
                error_rate_testing_data.append(test_error)
                time_taken_temp.append(temp_time1)
            else:
        ...
        for training_size in training_sizes:
            X_train_temp, y_train_temp = split_data(X_train_german, y_train_german, training_size, 6)
            if training_size == 1.0:
                train_error, temp_time1 = dt(X_train_temp, y_train_temp, X_train_temp, y_train_temp, recall_score, 5, 0,
                                             15, 0, 0)
                error_rate_training_data.append(train_error)
                test_error, temp_time2 = dt(X_train_temp, y_train_temp, X_test_german, y_test_german, recall_score, 5,
                                            0, 15, 1, 0)
                error_rate_testing_data.append(test_error)
                time_taken_temp.append(temp_time1)
            else:
        ...
        for training_size in training_sizes:
            X_train_temp, y_train_temp = split_data(X_train_german, y_train_german, training_size, 6)
            if training_size == 1.0:
                train_error, temp_time1 = svc(X_train_temp, y_train_temp, X_train_temp, y_train_temp, recall_score, 5,
                                              1, ['linear', 'poly', 'rbf', 'sigmoid'], 15, 0, 0)
                error_rate_training_data.append(train_error)
                test_error, temp_time2 = svc(X_train_temp, y_train_temp, X_test_german, y_test_german, recall_score, 5,
                                             1, ['linear', 'poly', 'rbf', 'sigmoid'], 15, 1, 0)
                error_rate_testing_data.append(test_error)
                time_taken_temp.append(temp_time1)
            else:
                train_error, temp_time1 = svc(X_train_temp, y_train_temp, X_train_temp, y_train_temp, recall_score, 5,
        ...
        for training_size in training_sizes:
            X_train_temp, y_train_temp = split_data(X_train_german, y_train_german, training_size, 6)
            if training_size == 1.0:
                train_error, temp_time1 = nn(X_train_temp, y_train_temp, X_train_temp, y_train_temp, recall_score, 5, 0,
                                             15, 0, 0)
                error_rate_training_data.append(train_error)
                test_error, temp_time2 = nn(X_train_temp, y_train_temp, X_test_german, y_test_german, recall_score, 5,
                                            0, 15, 1, 0)
                error_rate_testing_data.append(test_error)
                time_taken_temp.append(temp_time1)
            else:
...
plt.legend()
plt.title("Training Time for German Credit Data")
